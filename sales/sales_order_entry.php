...
//SonDang test
//$loc_details = get_loc_details(_PHONE);
//            while ($myrow = db_fetch($loc_details)){
//                $quantity_on_hand = get_qoh_on_date(_PHONE, $myrow["loc_code"]);
//            }
//bug($quantity_on_hand);die;
...
function check_item_data()
{
	global $SysPrefs, $allow_negative_prices;
        ...
        //SonDang 08-May-2015 Get total quantity on hand of product 
        
            $loc_details = get_loc_details($_POST['stock_id']);
            while ($myrow = db_fetch($loc_details)){
                $quantity_on_hand = get_qoh_on_date($_POST['stock_id'], $myrow["loc_code"]);
            }
        //-----------------
	...
	if(!get_post('stock_id_text', true)) {
        ...
        //SonDang, check if sales order > quantity on stock. "Allow Negative Inventory" setting in setup -> System and General GL Setup
        
        elseif (($quantity_on_hand - $_POST['qty']) < 0) {
            set_focus('qty');
            display_error(_("So luong Quantity ban nhap nhieu hon so voi so quantity hien dang co trong kho => ko the giao dich"));
            return false;
        }
        //-------------------------------------------------------------------------------//
...
function handle_update_item()
{
	if ($_POST['UpdateItem'] != '' && check_item_data()) {
		$_SESSION['Items']->update_cart_item_new(get_post('tax_type_id'),$_POST['LineNo'],
...
function handle_new_item()
{
	...
	add_to_order_new(get_post('tax_type_id'),$_SESSION['Items'], get_post('stock_id'), input_num('qty'),
