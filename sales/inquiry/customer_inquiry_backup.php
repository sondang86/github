...
***********************************************************************/
require_once('refactoring_customer_inquiry.php'); //Toàn bộ functions sẽ nằm ở đây
...
$customer_inquiry->get_filtertype();
//bug($customer_inquiry->get_filtertype());die;
...
if (!@$_GET['popup']){
	...
	$buttonAddNew = null;
	if( $customer_inquiry->get_filtertype() !=0 ){
		switch ($customer_inquiry->get_filtertype()){
...
//bug($customer_inquiry->get_customer_id());die;
...
cust_allocations_list_cells(null, 'filterType', $customer_inquiry->get_filtertype(), true);
...
$cols = array(
	...
	_("RB") => array('align'=>'right', 'type'=>'amount'),
		...
		array('insert'=>true, 'fun'=>'prt_link') //những function này đều nằm ở refactoring_customer_inquiry.php
	);
